...
convertArgs (reg0, blk0, (x:xs)) = 
    let (reg1, blk1, blkList1) = convertExp(reg0, blk0, x)
        (reg2, blk2, blkList2, keyRegs) = convertArgs(reg1, blk1, xs)
    in (reg2, blk2, blkList1 ++ blkList2, [C.Reg (reg1 - 1)] ++ keyRegs)
...
convertExp (reg0, blk0, (S.XOpUn op expr)) =
    ...
    in (
        (reg1 + 1),
        (blk1),
        blkList ++ [C.Block blk1 [C.IArith (convertOpUn op) (C.Reg reg1) (C.Reg (reg1 - 1)) (C.Reg (reg1 - 1))]]
    )
convertExp (reg0, blk0, (S.XTernary cond expr1 expr2)) =
    let (reg1, blk1, blkList) = convertStmt (reg0, blk0, S.SIfElse cond (S.Block [S.SExp expr1]) (S.Block [S.SExp expr2]))
    in (
        (reg1),
        (blk1),
        blkList
